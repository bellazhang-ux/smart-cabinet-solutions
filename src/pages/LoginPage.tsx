      ...
      >
        <div className="bg-card rounded-lg border shadow-lg p-8">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-bold font-mono text-foreground">{t('app.name')}</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="relative">
              ...
              <Input placeholder={t('login.username')} value={username} onChange={e => setUsername(e.target.value)} className="pl-10" />
            </div>
            <div className="relative">
              <KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input type="password" placeholder={t('login.password')} value={password} onChange={e => setPassword(e.target.value)} className="pl-10" />
            </div>
            {error && <p className="text-destructive text-sm">{error}</p>}
            <Button type="submit" className="w-full gradient-primary">{t('login.signin')}</Button>
          </form>

          <div className="mt-6 pt-4 border-t">
            <p className="text-xs text-muted-foreground mb-2">{t('login.demo')}</p>
